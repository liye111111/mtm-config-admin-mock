...
export async function createProductBinding(request: Request, input: SaveProductBindingInput) { const template = await validateBinding(input); const product = await resolveShopifyProduct(request, input); if (product.status === "ARCHIVED") throw new AppError("已归档商品不能启用定制", 422); if (await products.findByProduct(product.shopId, product.gid)) throw new AppError("该 Shopify 商品已经绑定定制模板", 409); await setShopifyCustomizationMarker(request, product.gid, product.shopId, customizationMarker(input, template)); try { return productBindingView(await products.createProductBinding(input, product)); } catch (error) { await setShopifyCustomizationMarker(request, product.gid, product.shopId, { enabled: false }).catch(() => undefined); throw error; } }
