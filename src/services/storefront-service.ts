...
export async function createCustomization(shopId: string, input: CreateCustomizationInput) { const existing = await customizations.findByIdempotencyKey(shopId, input.idempotencyKey); if (existing) return instanceResponse(existing); const { row, summary } = await validateAuthoritatively(shopId, input); try { const created = await customizations.createCustomizationInstance({ shopId, input, templateId: row.id, templateCode: row.code, templateVersion: row.version, schemaVersion: row.schema_version, summary }); if (!created) throw new AppError("定制实例创建失败", 500); return instanceResponse(created); } catch (error) { const concurrent = await customizations.findByIdempotencyKey(shopId, input.idempotencyKey); if (concurrent) return instanceResponse(concurrent); throw error; } }
