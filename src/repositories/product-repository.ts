...
export async function createProductBinding(input: SaveProductBindingInput) {
  ...
  await database().prepare("INSERT INTO product_bindings (id,shopify_product_id,product_title,product_handle,template_id,published_version,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)").bind(id, input.shopifyProductId, input.productTitle, input.productHandle, input.templateId, input.publishedVersion, now, now).run();
  return (await findProductBinding(id))!;
}
export async function updateProductBinding(id: string, input: SaveProductBindingInput) {
  await ensureDatabase(); await database().prepare("UPDATE product_bindings SET shopify_product_id=?,product_title=?,product_handle=?,template_id=?,published_version=?,updated_at=? WHERE id=?").bind(input.shopifyProductId, input.productTitle, input.productHandle, input.templateId, input.publishedVersion, new Date().toISOString(), id).run();
