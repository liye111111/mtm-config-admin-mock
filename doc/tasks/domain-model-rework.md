...
7. 使用纯新版 Schema v2，不保留旧模板兼容代码。
8. 保持 Storefront API 路径稳定，主题与新版响应结构同步更新。
...
- 非 Schema v2 模板直接拒绝，不把任意对象断言为新版模板。
...
2. 删除旧 `pieces/pieceSelection` 模型和相关代码。
3. 增加 D1 字段、新表、索引和 Drizzle Schema。
4. Repository 只读取 Schema v2 配置。
5. 管理端保存草稿时写 Schema v2。
6. 发布模板时校验所有编码、关系和尺寸范围。
7. Storefront API 输出兼容现有主题的数据，并逐步增加新版字段。
8. 在切换窗口备份旧 D1，并初始化新版模板数据。
9. 旧数据如需保留，由业务按新版模型重新录入，不增加运行时兼容逻辑。
...
- 管理端只打开 Schema v2 模板，旧 Schema 返回明确错误；
- Storefront API 不返回草稿、内部审计字段或客户隐私数据；
- Storefront API 统一使用 `enabled + configuration` 响应，不再通过404表示没有定制功能；
- `configuration` 内提供 `templateId`、`version`、`components`、`steps` 和 `measurementBlocks`；
...
控制：切换前备份旧 D1；只接受 Schema v2；旧数据按新版模型重新录入。
...
- 所有数据库 JSON 读取均经过 Schema v2 Zod 解析，旧 Schema 被明确拒绝；
