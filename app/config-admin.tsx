...
export function ConfigAdmin() {
  ...
  function newBinding() { const templateId = items.find((item) => item.status === "published")?.id ?? ""; setEditingBinding({ id: "", shopId: "", shopifyProductGid: "", shopifyProductId: "", productTitle: "", productHandle: "", productStatus: "ACTIVE", productKind: "single", variantCount: 0, templateId, publishedVersion: null, enabled: true, syncStatus: "stale" }); void loadBindingVersions(templateId); }
  function editBinding(binding: ProductBindingView) { setEditingBinding({ ...clone(binding), mockProduct: { gid: binding.shopifyProductGid, title: binding.productTitle, handle: binding.productHandle, imageUrl: binding.productImageUrl, imageAlt: binding.productImageAlt, status: binding.productStatus, variantCount: binding.variantCount, onlineStoreUrl: binding.onlineStoreUrl, updatedAt: binding.shopifyUpdatedAt } }); void loadBindingVersions(binding.templateId); }
  async function pickProduct() { const product = await selectShopifyProduct(); if (!product || !editingBinding) return; setEditingBinding({ ...editingBinding, shopifyProductGid: product.gid, shopifyProductId: product.gid.split("/").at(-1) ?? "", productTitle: product.title, productHandle: product.handle, productImageUrl: product.imageUrl, productImageAlt: product.imageAlt, productStatus: product.status, variantCount: product.variantCount, onlineStoreUrl: product.onlineStoreUrl, mockProduct: product }); }
...
function CustomerProfiles({ profiles, bindings, templates, draft, onDraft, onNew, onEdit, onDelete, onSave, onCancel, onRefresh }: { profiles: MeasurementProfileAdminView[]; bindings: ProductBindingView[]; templates: TemplateView[]; draft: CustomerProfileDraft | null; onDraft: (draft: CustomerProfileDraft | null) => void; onNew: () => void; onEdit: (profile: MeasurementProfileAdminView) => void; onDelete: (profile: MeasurementProfileAdminView) => void; onSave: () => void; onCancel: () => void; onRefresh: () => void }) {
  ...
  return <>
    ...
    <div className="panel table-wrap"><table><thead><tr><th>归属</th><th>客户/设备</th><th>店铺</th><th>单位</th><th>字段</th><th>更新时间</th><th>过期时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{profiles.map((profile) => {
      ...
      return <tr key={profile.id}><td><span className={`badge ${profile.ownerType === "customer" ? "published" : "draft"}`}>{profile.ownerType === "customer" ? "登录客户" : "匿名设备"}</span></td><td><strong>{profile.customerId ? (profile.customerName || `Customer ${profile.customerId}`) : `Guest …${profile.id.slice(-8)}`}</strong>{profile.customerEmail && <small>{profile.customerEmail}</small>}<small>{profile.customerId ? `Customer ${profile.customerId} · ` : ""}Schema v{profile.schemaVersion}</small></td><td>{profile.shopId}</td><td>{profile.unit}</td><td>{profile.fieldCount} 项</td><td>{new Date(profile.updatedAt).toLocaleString("zh-CN")}</td><td>{profile.expiresAt ? new Date(profile.expiresAt).toLocaleString("zh-CN") : "账号长期保存"}</td><td><span className={`badge ${expired ? "draft" : "published"}`}>{expired ? "已过期" : "有效"}</span></td><td><button className="link" onClick={() => onEdit(profile)}>编辑</button><button className="link danger-text" onClick={() => onDelete(profile)}>删除</button></td></tr>;
...
type ProductBindingsProps = { items: TemplateView[]; bindings: ProductBindingView[]; editing: ProductBindingView | null; versions: TemplateVersionView[]; embedded: boolean; onPick: () => void; onNew: () => void; onEdit: (binding: ProductBindingView) => void; onRemove: (binding: ProductBindingView) => void; onSync: (binding: ProductBindingView) => void; onPreview: (binding: ProductBindingView) => void; onChange: (binding: ProductBindingView) => void; onTemplateChange: (templateId: string) => void; onCancel: () => void; onSave: () => void };

function ProductBindingsNew({ items, bindings, editing, versions, embedded, onPick, onNew, onEdit, onRemove, onSync, onPreview, onChange, onTemplateChange, onCancel, onSave }: ProductBindingsProps) {
  ...
  return <>
    ...
    {editing && <div className="panel binding-form">
      ...
      <div className="product-picker-card">
        {editing.shopifyProductGid ? <>{editing.productImageUrl ? <img src={editing.productImageUrl} alt={editing.productImageAlt || ""}/> : <div className="product-placeholder">商品</div>}<div><strong>{editing.productTitle}</strong><small>{editing.shopifyProductId} · {editing.productHandle || "无 Handle"}</small><small>{editing.productStatus} · {editing.variantCount} 个 Variants</small></div><button className="secondary" onClick={onPick}>重新选择商品</button></> : <><div><strong>尚未选择 Shopify 商品</strong><small>商品名称、主图和状态将由 Shopify 提供</small></div><button className="secondary" onClick={onPick}>选择 Shopify 商品</button></>}
      </div>
      <div className="form">
      ...
      <div className="actions"><button className="secondary" onClick={onCancel}>取消</button><button className="primary" disabled={!editing.shopifyProductGid || !editing.templateId} onClick={onSave}>保存绑定</button></div>
    </div>}
