...
function CustomerProfiles({ profiles, bindings, templates, draft, onDraft, onNew, onEdit, onDelete, onSave, onCancel, onRefresh }: { profiles: MeasurementProfileAdminView[]; bindings: ProductBindingView[]; templates: TemplateView[]; draft: CustomerProfileDraft | null; onDraft: (draft: CustomerProfileDraft | null) => void; onNew: () => void; onEdit: (profile: MeasurementProfileAdminView) => void; onDelete: (profile: MeasurementProfileAdminView) => void; onSave: () => void; onCancel: () => void; onRefresh: () => void }) {
  const [profileFilter, setProfileFilter] = useState<"all" | "customer" | "guest" | "activeGuest">("all");
  ...
  const isActiveGuest = (profile: MeasurementProfileAdminView) => profile.ownerType === "guest" && Boolean(profile.expiresAt && new Date(profile.expiresAt) > new Date());
  const activeGuestCount = profiles.filter(isActiveGuest).length;
  const filteredProfiles = profiles.filter((profile) => profileFilter === "all" || (profileFilter === "customer" && profile.ownerType === "customer") || (profileFilter === "guest" && profile.ownerType === "guest") || (profileFilter === "activeGuest" && isActiveGuest(profile)));
  const filterLabel = { all: "全部资料", customer: "登录客户", guest: "匿名资料", activeGuest: "有效匿名资料" }[profileFilter];
  function chooseFilter(filter: typeof profileFilter) { setProfileFilter((current) => current === filter ? "all" : filter); }
  ...
  return <>
    ...
    <section className="stats" aria-label="客户资料筛选"><Stat label="资料总数" value={profiles.length} active={profileFilter === "all"} onClick={() => chooseFilter("all")}/><Stat label="登录客户" value={customerCount} active={profileFilter === "customer"} onClick={() => chooseFilter("customer")}/><Stat label="匿名资料" value={guestCount} active={profileFilter === "guest"} onClick={() => chooseFilter("guest")}/><Stat label="有效匿名资料" value={activeGuestCount} active={profileFilter === "activeGuest"} onClick={() => chooseFilter("activeGuest")}/></section>
    ...
    <div className="customer-filter-result"><strong>{filterLabel}</strong><span>共 {filteredProfiles.length} 条</span>{profileFilter !== "all" && <button className="link" onClick={() => setProfileFilter("all")}>清除筛选</button>}</div>
    <div className="panel table-wrap"><table><thead><tr><th>归属</th><th>客户/设备</th><th>店铺</th><th>单位</th><th>字段</th><th>更新时间</th><th>过期时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{filteredProfiles.map((profile) => {
    ...
    })}</tbody></table>{!filteredProfiles.length && <div className="empty">{profiles.length ? "当前筛选条件下暂无客户资料。" : "暂无已保存的量体资料。可先通过商品定制器保存一条测试资料。"}</div>}</div>
