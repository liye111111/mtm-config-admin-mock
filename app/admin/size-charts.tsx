...
export function SizeCharts({ measurementAttributes }: { measurementAttributes: MeasurementAttribute[] }) {
  ...
  return <>
    ...
    <div className="work size-chart-work">
      <aside className="list"><div className="list-search"><input value={search} placeholder="搜索尺码表" onChange={(event) => setSearch(event.target.value)}/></div>{filtered.map((item) => <button key={item.id} className={`list-item ${item.id === selectedId ? "active" : ""}`} onClick={() => choose(item)}><strong>{item.name}</strong><small>{item.code}</small><span className={`badge ${item.status}`}>{item.status === "active" ? "启用" : "停用"}</span><span className="version-badge">{item.currentVersion ? `已发布 v${item.currentVersion}` : "未发布"}</span></button>)}{!filtered.length && <div className="empty">暂无尺码表</div>}</aside>
...
function ConfigurationTab({ config, attributes, onConfig, onInputs, onSizes }: { config: SizeChartConfig; attributes: MeasurementAttribute[]; onConfig: (updater: (next: SizeChartConfig) => void) => void; onInputs: (items: SizeChartInputAttribute[]) => void; onSizes: (items: SizeDefinition[]) => void }) {
  ...
  return <div className="size-chart-config">
    <section className="config-section"><div className="section-title"><div><h4>推荐算法</h4><p>算法由代码实现并版本化，页面只维护算法所需的数据。</p></div></div><div className="algorithm-grid">{algorithms.map(([code, item]) => <button key={code} className={`algorithm-card ${config.algorithm.code === code ? "selected" : ""}`} onClick={() => switchAlgorithm(code)}><strong>{item.name}</strong><small>{code}@1</small><span>{item.summary}</span></button>)}</div><div className="algorithm-explanation"><strong>{info.name}的计算逻辑</strong><ol>{info.calculation.map((line) => <li key={line}>{line}</li>)}</ol></div></section>
    ...
    <section className="config-section"><div className="section-title"><div><h4>可推荐尺码</h4><p>尺码编码用于算法输出，名称用于后台和前台展示。</p></div><button className="secondary" onClick={() => onSizes([...config.sizes, { code: "", label: "", sortOrder: config.sizes.length * 10 + 10 }])}>＋ 添加尺码</button></div><div className="size-definition-table">{config.sizes.map((size, index) => <div className="size-definition-row" key={`${index}-${size.code}`}><span>{index + 1}</span><input value={size.code} placeholder="175_92A" onChange={(event) => { const next = clone(config.sizes); next[index].code = event.target.value; onSizes(next); }}/><input value={size.label} placeholder="175/92A" onChange={(event) => { const next = clone(config.sizes); next[index].label = event.target.value; onSizes(next); }}/><button className="delete" onClick={() => onSizes(config.sizes.filter((_, itemIndex) => itemIndex !== index))}>删除</button></div>)}</div></section>
