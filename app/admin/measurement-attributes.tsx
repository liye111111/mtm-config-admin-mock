...
export function MeasurementAttributes({ items, draft, onDraft, onSave, onDelete, onToggle, onRefresh }: {
  ...
  return <>
    ...
    {draft && <section className="panel binding-form measurement-attribute-form"><div className="form">
      ...
      <Field label="最小值"><input type="number" step="any" value={draft.min} disabled={draft.valueType === "enum"} onChange={(event) => onDraft({ ...draft, min: Number(event.target.value) })}/></Field>
      <Field label="最大值"><input type="number" step="any" value={draft.max} disabled={draft.valueType === "enum"} onChange={(event) => onDraft({ ...draft, max: Number(event.target.value) })}/></Field>
      <Field label="滑动步长"><input type="number" min="0.000001" step="any" value={draft.step} disabled={draft.valueType === "enum"} onChange={(event) => onDraft({ ...draft, step: Number(event.target.value) })}/></Field>
      ...
      <div className="measurement-attribute-description"><ImageField label="测量示例图" image={draft.image} onChange={(image) => onDraft({ ...draft, image })}/></div>
